...
- **Copilot real token usage via `--usage-output-file`**: discovered during T6 implementation that the installed Copilot CLI (1.0.82) can write usage to a file via `--usage-output-file`, which the current `CliProviderSpec` contract (stdout-only) doesn't read. Today Copilot correctly reports `tokensAvailable: false` per spec (graceful degradation is spec-compliant), but reading that file would give real numbers instead of "n/a". Left out of this feature's scope — the spec only requires graceful degradation, not exhausting every possible enhancement — but worth a fast-follow if real Copilot token counts matter later.
- **Per-tool timeout tuning**: `DEFAULT_TIMEOUT_MS` (120s) is shared across all 4 CLI providers, inherited unchanged from `ClaudeCodeProvider`. Codex/Copilot/Kiro run full agentic turns (not a pure chat completion), so a large `gw review` diff could plausibly hit this timeout for those providers more often than it did for Claude. Not a regression (same timeout Claude always had), but a candidate follow-up if timeouts are reported in practice.
