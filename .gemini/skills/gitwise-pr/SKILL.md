---
name: gitwise-pr
description: Generates a PR title and body from the diff and creates/updates the GitHub PR using `gh`. Use when asked to open a pull request, draft a PR, or create a PR.
---

# gitwise-pr

## Instructions

1. Execute the PR runner:
   `node packages/skills/dist/scripts/pr.js [--base <branch>]`
2. Display the emitted markdown plan (title + body) to the user.
3. Ask for confirmation. If the user approves:
   - Run: `node packages/skills/dist/scripts/pr.js [--base <branch>] --apply`
4. Show the PR URL returned.

## Flags
- `--base <branch>` — base branch for diff (default: auto-detect main/master)
- `--apply` — create or update the GitHub PR
- `--prompt "<text>"` — additional focus instructions for the PR drafter
