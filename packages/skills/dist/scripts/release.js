...
function isPersistedReleasePlan(value) {
  ...
  const p2 = value;
  if (p2.schema !== 1) return false;
  if (p2.strategy !== "gitflow" && p2.strategy !== "github-flow") return false;
  if (p2.suggestedBump !== "major" && p2.suggestedBump !== "minor" && p2.suggestedBump !== "patch") {
    return false;
  }
  if (typeof p2.currentVersion !== "string") return false;
  if (typeof p2.newVersion !== "string") return false;
  if (typeof p2.changelog !== "string") return false;
  if (typeof p2.notes !== "string") return false;
  if (typeof p2.commits !== "string") return false;
  if (typeof p2.preparedAt !== "string") return false;
  if (typeof p2.baseCommit !== "string") return false;
  if (typeof p2.targetBranch !== "string") return false;
  if (typeof p2.releaseBranchCreated !== "boolean") return false;
  if (!p2.tokens || typeof p2.tokens !== "object") return false;
  const tokens = p2.tokens;
...
async function prepareRelease(opts) {
  ...
  try {
    const dirtyEntries = (await status(cwd)).split("\n").map((line) => line.replace(/\s+$/, "")).filter((line) => line.length >= 3).filter((line) => {
      ...
      if (path4 === ".gitignore") return false;
      if (path4 === ".gitwise/" || path4 === ".gitwise") return false;
      if (path4.startsWith(".gitwise/")) return false;
      return true;
    });
...
async function readWorkspacePatterns(cwd) {
  ...
  const fromArray = Array.isArray(ws) ? ws.filter((p2) => typeof p2 === "string" && p2.length > 0) : [];
  if (fromArray.length > 0) return fromArray;
  if (ws && typeof ws === "object" && !Array.isArray(ws)) {
    const inner = ws.packages;
    if (Array.isArray(inner)) {
      const fromObject = inner.filter(
        (p2) => typeof p2 === "string" && p2.length > 0
      );
...
async function expandWorkspacePatterns(cwd, patterns) {
  ...
  for (const pattern of patterns) {
    ...
    const segments = pattern.split("/").filter((s2) => s2.length > 0);
