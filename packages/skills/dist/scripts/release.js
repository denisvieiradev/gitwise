...
async function prepareRelease(opts) {
  ...
  try {
    const dirtyEntries = (await status(cwd)).split("\n").map((line) => line.replace(/\s+$/, "")).filter((line) => line.length >= 3).filter((line) => {
      const path14 = line.slice(3).trim();
      if (path14 === ".gitignore") return false;
      if (path14 === ".gitwise/" || path14 === ".gitwise") return false;
      if (path14.startsWith(".gitwise/")) return false;
      return true;
    });
