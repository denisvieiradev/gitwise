...
async function prepareRelease(opts) {
  ...
  try {
    const dirtyEntries = (await status(cwd)).split("\n").map((line) => line.replace(/\s+$/, "")).filter((line) => line.length >= 3).filter((line) => {
      const path11 = line.slice(3).trim();
      if (path11 === ".gitignore") return false;
      if (path11 === ".gitwise/" || path11 === ".gitwise") return false;
      if (path11.startsWith(".gitwise/")) return false;
      return true;
    });
