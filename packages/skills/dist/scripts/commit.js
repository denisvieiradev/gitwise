...
async function getStagedFiles(cwd) {
  const files = await parseStatus(cwd);
  return files.filter((f2) => f2.indexStatus !== " " && f2.indexStatus !== "?");
}
...
async function getUnstagedFiles(cwd) {
  const files = await parseStatus(cwd);
  return files.filter(
    (f2) => f2.indexStatus === "?" && f2.workTreeStatus === "?" || f2.workTreeStatus !== " "
  );
}
...
async function showFileAtHead(cwd, path4) {
  debug("git command", { args: ["show", `HEAD:${path4}`], cwd });
  try {
    const result = await exec("git", ["show", `HEAD:${path4}`], {
      cwd,
      timeout: GIT_TIMEOUT_MS,
  ...
  async rollback(reason, logger) {
    ...
    if (failures.length > 0) {
      logger.warn("rollback partial: one or more compensate actions failed", {
        ...
        failures: failures.map((f2) => ({
          step: f2.step,
          error: serializeError(f2.error)
        }))
      });
    }
...
function isSensitiveFile(filePath) {
  ...
  return SENSITIVE_PATTERNS.some((pattern) => pattern.test(basename2));
}
...
function extractBalancedJsonCandidates(raw) {
  ...
  for (let i2 = 0; i2 < raw.length; i2++) {
    ...
    if (c2 === "{") {
      ...
      if (start !== void 0) {
        candidates.push(raw.slice(start, i2 + 1));
      }
    }
  }
  return candidates;
...
function parseAlternativesResponse(raw) {
  const isValid = (p2) => typeof p2 === "object" && p2 !== null && p2["type"] === "alternatives" && Array.isArray(p2["options"]) && p2["options"].length > 0 && p2["options"].every((o2) => typeof o2 === "string");
  try {
...
function parseCommitResponse(raw) {
  ...
  const parsedCandidates = candidates.map(tryParseJson).filter((p2) => p2 !== null);
  const plan = parsedCandidates.find((p2) => p2.type === "plan");
  if (plan) return plan;
...
IMPORTANT: Generate exactly 3 different alternative commit messages. Return JSON only: {"type": "alternatives", "options": ["message1", "message2", "message3"]}` : ""
  ...
  if (split === "never") {
    const message2 = parsed.type === "single" ? parsed.message : parsed.commits.map((c2) => c2.message).join("\n\n");
    return {
      kind: "single",
...
var AnthropicProvider = class {
  ...
  isRetryable(err) {
    if (err instanceof sdk_default.APIError) {
...
async function main() {
  ...
  const plan = result;
  if (plan.kind === "single") {
    const c2 = plan.commits[0];
    if (!c2) throw new Error("commit() returned a single plan with no commits");
...
`);
    if (c2.files && c2.files.length > 0) {
      process.stdout.write(`**Files:** ${c2.files.join(", ")}
...
`);
    for (let i2 = 0; i2 < plan.commits.length; i2++) {
      const c2 = plan.commits[i2];
      if (!c2) continue;
      process.stdout.write(`### Commit ${i2 + 1}
**Message:** ${c2.message}
`);
      if (c2.files && c2.files.length > 0) {
        process.stdout.write(`**Files:** ${c2.files.join(", ")}
