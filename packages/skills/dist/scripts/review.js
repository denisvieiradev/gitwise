...
function extractSection(markdown, heading) {
  ...
  return match[1].split("\n").map((l2) => l2.replace(/^[-*•]\s*/, "").trim()).filter((l2) => l2.length > 0);
}
...
function buildMarkdown(parsed) {
  ...
  if (parsed.critical.length > 0) {
    sections.push(...parsed.critical.map((f2) => `- ${f2.description}`));
  } else {
    sections.push("_No critical issues found._");
  }
  sections.push("\n## Suggestions");
  if (parsed.suggestions.length > 0) {
    sections.push(...parsed.suggestions.map((f2) => `- ${f2.description}`));
  } else {
    sections.push("_No suggestions._");
  }
  sections.push("\n## Nitpicks");
  if (parsed.nitpicks.length > 0) {
    sections.push(...parsed.nitpicks.map((f2) => `- ${f2.description}`));
  } else {
...
async function main() {
  ...
  if (result.critical.length > 0) {
    ...
    for (const f2 of result.critical) {
      const prefix = f2.file ? `**${f2.file}** ` : "";
      process.stdout.write(`- ${prefix}${f2.description}
`);
    }
  ...
  if (result.suggestions.length > 0) {
    ...
    for (const f2 of result.suggestions) {
      const prefix = f2.file ? `**${f2.file}** ` : "";
      process.stdout.write(`- ${prefix}${f2.description}
`);
    }
  ...
  if (result.nitpicks.length > 0) {
    ...
    for (const f2 of result.nitpicks) {
      const prefix = f2.file ? `**${f2.file}** ` : "";
      process.stdout.write(`- ${prefix}${f2.description}
`);
    }
