...
- **Diffs are sent to the vendor behind the `provider` you configure**, and only to that vendor: Anthropic for `api` (the Anthropic API) and `claude-code` (your local Claude Code subprocess), OpenAI for `codex` (OpenAI Codex CLI), GitHub for `copilot` (GitHub Copilot CLI), and AWS for `kiro` (AWS Kiro CLI). These LLM calls are the only data that leaves your machine. There is no other telemetry. See the README "Privacy" section for the full posture.
- **`gh` and the provider CLI binaries (`claude`, `codex`, `copilot`, `kiro-cli`)** are invoked as subprocesses with no `shell: true`; arguments are passed as array elements to avoid command-injection surfaces.
